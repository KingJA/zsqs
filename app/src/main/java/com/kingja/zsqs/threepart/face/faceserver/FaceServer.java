package com.kingja.zsqs.threepart.face.faceserver;
...
import com.kingja.zsqs.threepart.face.model.FaceRegisterInfo;
...
public class FaceServer {
    ...
    public boolean registerNv21(Context context, byte[] nv21, int width, int height, FaceInfo faceInfo, String name) {
        synchronized (this) {
            ...
            //特征提取
            int code = faceEngine.extractFaceFeature(nv21, width, height, FaceEngine.CP_PAF_NV21, faceInfo,
                    faceFeature);
    ...
    public FaceFeature getFaceFeature(Context context, byte[] nv21, int width, int height, FaceInfo faceInfo) {
        synchronized (this) {
            if (faceEngine == null || context == null || nv21 == null || width % 4 != 0 || nv21.length != width * height * 3 / 2) {
                Log.e(TAG, "registerNv21: invalid params");
                return null;
    ...
    public boolean registerUser(Context context, Bitmap faceBitmap, FaceFeature faceFeature, String name) {
        synchronized (this) {
            if (ROOT_PATH == null) {
                ROOT_PATH = context.getFilesDir().getAbsolutePath();
            }
            ...
            try {
                File file = new File(imgDir + File.separator + userName + IMG_SUFFIX);
                FileOutputStream fosImage = new FileOutputStream(file);
                faceBitmap.compress(Bitmap.CompressFormat.JPEG, 100, fosImage);
                fosImage.close();
    ...
    public Bitmap getPreviewFaceBitmap(byte[] nv21, int width, int height, Rect rect, int orient) {
        Rect cropRect = getBestRect(width, height, rect);
        cropRect.left &= ~3;
        cropRect.top &= ~3;
        cropRect.right &= ~3;
        cropRect.bottom &= ~3;
        return getHeadImage(nv21, width, height, orient, cropRect, ArcSoftImageFormat.NV21);
    }
    ...
    private Bitmap getHeadImage(byte[] originImageData, int width, int height, int orient, Rect cropRect,
        ...
        if (rotateDegree != null) {
            ...
            rotateCode = ArcSoftImageUtil.rotateImage(headImageData, rotateHeadImageData, cropRect.width(),
